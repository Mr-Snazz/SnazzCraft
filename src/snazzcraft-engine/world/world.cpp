...
SnazzCraft::World::World(std::string IName, uint32_t ISize, int32_t ISeed)
    : Name(IName), Seed(ISeed), ThreadPool(SnazzCraft::ThreadPoolBasic(6u)), Chunks(std::unordered_map<uint64_t, SnazzCraft::Chunk*>()), VoxelPlacementDisplayPosition(glm::vec3(0.0f))
{
    this->Size = static_cast<int32_t>((ISize > SnazzCraft::World::MaxSize || !ISize) ? SnazzCraft::World::MaxSize : ISize);
...
{
    return Chunk && Chunk->Position[0] >= -this->Size && Chunk->Position[0] <= this->Size && Chunk->Position[1] >= -this->Size && Chunk->Position[1] <= this->Size;
}
