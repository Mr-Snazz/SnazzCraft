...
{
    this->GUIInputHandler = new SnazzCraft::InputHandler(Window);

    this->WidgetCollections.push_back(new SnazzCraft::WidgetCollection());
    this->ActiveWidgetCollectionIndex = 1;

    this->WidgetCollections[1]->Panels.push_back(SNAZZCRAFT_GENERATE_PANEL_LONG(10, 10, new SnazzCraft::Texture("textures/gui/button.png"))); 

    this->WidgetCollections[1]->Buttons.push_back(SNAZZCRAFT_GENERATE_BUTTON_LONG(10, 10));
